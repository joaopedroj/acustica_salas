...
for j in Lista_ATs:
    if j == 9:
        for i in range(1,17):
            ...
            fs, audio = wav.read(f"AT{j}_explosão{i}.wav")
            ...
            for f_central in f_central_lista:
                ...
                c_50.append(C50)
                d_50.append(D50)
                print(f"AT{j} impulso {i} - Banda {f_central} Hz: D50 ≈ {D50*100:.3f}%")
                print(f"AT{j} impulso {i} - Banda {f_central} Hz: C50 ≈ {C50:.3f}")
    elif j==10:
        for i in range(1,9):
            # Carregamento do .wav
            # fs, audio = wav.read("01dB_explosao.wav")
            fs, audio = wav.read(f"AT{j}_explosao{i}.wav")
            audio = audio / np.max(np.abs(audio))  # Normalização
            t = np.arange(0, (len(audio)-1)/fs + 1/fs, 1/fs)
