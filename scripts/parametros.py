...
_DEFAULT_AUDIO = os.path.join(_AUDIO_DIR, 'AT10_impulse_03.wav')
...
# Carregamento do .wav
# fs, audio = wav.read(os.path.join(_AUDIO_DIR, 'AT10_impulse_01dB.wav'))
fs, audio_raw = wav.read(_DEFAULT_AUDIO)
audio = audio_raw / np.max(np.abs(audio_raw))  # Normalização
